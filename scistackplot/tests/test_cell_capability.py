"""What the panel is told about a 1-D measure whose cells can be collapsed.
...
from scistackplot.roles import Assignment, default_assignment

SAMPLE = {"trial": Role.COLLAPSE}
NO_SAMPLE = {"trial": Role.ITERATE}
...
def test_a_1d_measure_offers_the_scalar_kinds_when_collapsible():
    kinds = available_plots(Shape.SERIES_1D, SAMPLE, collapsible=True)
...
def test_without_collapsible_the_1d_list_is_unchanged():
    assert available_plots(Shape.SERIES_1D, SAMPLE) == [
        PlotKind.LINE,
        PlotKind.BAND,
    ]


def test_the_distribution_kinds_still_need_a_sample():
    kinds = available_plots(Shape.SERIES_1D, NO_SAMPLE, collapsible=True)
    ...
    reason = why_unavailable(
        PlotKind.VIOLIN, Shape.SERIES_1D, NO_SAMPLE, collapsible=True
    )
    assert reason and "sample" in reason
...
def test_an_offered_kind_carries_no_refusal():
    ...
    for kind in available_plots(Shape.SERIES_1D, SAMPLE, collapsible=True):
        assert (
            why_unavailable(kind, Shape.SERIES_1D, SAMPLE, collapsible=True) is None
        )


def test_a_relational_plot_is_unaffected():
    kinds = available_plots(
        Shape.SERIES_1D, SAMPLE, has_x_measure=True, collapsible=True
    )
...
def _violin(measure="Signal", **kwargs) -> PlotSpec:
    roles = kwargs.pop("roles", {"subject": Role.GROUP, "trial": Role.COLLAPSE})
...
def test_grouping_reads_as_ticks_once_collapsed(series_table):
    """A 1-D measure's x axis is its own index, so its grouping layers are
    series. Cell-collapsed, the measure is scalar and the same layers become
    ticks — the rule in ``grouping_layers`` is untouched; it simply sees a
    scalar measure."""
    grouping = capabilities(_violin(), series_table)["grouping"]
    assert grouping["ticks"] == ["subject"] and grouping["series"] == []

    line = PlotSpec(measures=["Signal"], kind=PlotKind.LINE, roles={"subject": Role.GROUP})
    grouping = capabilities(line, series_table)["grouping"]
    assert grouping["ticks"] == [] and grouping["series"] == ["subject"]
...
def test_choosing_a_scalar_kind_rerolls_untouched_1d_roles(series_table):
    """A 1-D measure opens with every key iterated, which leaves no sample —
    ...
    scalar_default = default_assignment(series_table, "Signal", shape=Shape.SCALAR)
    assert suggested.groups == scalar_default.groups == ["trial"]
    assert Role.COLLAPSE in suggested.roles.values(), "the distribution now has a sample"
...
def test_the_reroll_is_symmetric(series_table):
    ...
    collapsed = default_assignment(series_table, "Signal", shape=Shape.SCALAR).apply(
        PlotSpec(measures=["Signal"], kind=PlotKind.VIOLIN)
    )
    suggested = roles_for_kind(collapsed, series_table, PlotKind.LINE)
    assert suggested == default_assignment(series_table, "Signal", shape=Shape.SERIES_1D)


def test_no_reroll_when_the_shape_does_not_change(series_table, scalar_table):
    box = default_assignment(series_table, "Signal", shape=Shape.SCALAR).apply(
        PlotSpec(measures=["Signal"], kind=PlotKind.VIOLIN)
    )
    assert roles_for_kind(box, series_table, PlotKind.BOX) is None
    scalar = default_assignment(scalar_table, "StepLength").apply(
        PlotSpec(measures=["StepLength"])
    )
...
def test_the_report_carries_the_suggestion_per_kind(series_table):
    ...
    assert kinds["violin"]["assignment"], "the panel applies this synchronously"
...
def test_switching_kinds_does_not_share_a_plan(step_length_table):
    ...
    roles = {"subject": Role.GROUP, "trial": Role.GROUP}
    line = PlotSpec(measures=["StepLength"], kind=PlotKind.LINE, roles=roles, color="subject")
    violin = PlotSpec(measures=["StepLength"], kind=PlotKind.STRIP, roles=roles, color="subject")
...
#
# Both of these came out of one failing assertion (2026-09-14): the scalar
# default has no sample, so the re-roll handed back roles that STILL could not
# draw a violin. Looking at that showed the other half: from the opening state
# box/violin are refused for want of a sample, so the kind whose selection
# would have supplied one could not be clicked either.
...
def test_a_two_factor_table_collapses_the_key_outside_the_grouped_one():
    """The scalar default groups `session` (the deepest key) and iterates
    `subject` — one point per figure, nothing to distribute. The key just
    outside the grouped one is collapsed: the sample is the subjects, the
    marks stay per session."""
    table = _two_factor_table()
    opened = default_assignment(table, "Signal").apply(
        PlotSpec(measures=["Signal"], kind=PlotKind.LINE)
    )

    suggested = roles_for_kind(opened, table, PlotKind.VIOLIN)

    assert suggested == Assignment(
        roles={"subject": Role.COLLAPSE, "session": Role.GROUP}, groups=["session"]
    )


def test_a_one_factor_table_keeps_its_only_factor_grouped():
    """One point per violin is not a violin — but the only factor is the mark
    the user is looking at, and collapsing it would leave nothing to draw.
    Nothing is eligible, so the suggestion is the scalar default unchanged
    and the violin stays greyed out with its reason."""
    frame = pd.DataFrame(
    ...
    assert roles_for_kind(opened, table, PlotKind.VIOLIN) == Assignment(
        roles={"subject": Role.GROUP}, groups=["subject"]
    )


def test_a_non_distribution_kind_is_not_given_a_sample():
    """Only the kinds that distribute need a sample; a scatter is a figure
    with one point per record, and collapsing a factor for it would silently
    change what the marks mean."""
    table = _two_factor_table()
    opened = default_assignment(table, "Signal").apply(
        PlotSpec(measures=["Signal"], kind=PlotKind.LINE)
    )

    assert roles_for_kind(opened, table, PlotKind.SCATTER) == Assignment(
        roles={"subject": Role.ITERATE, "session": Role.GROUP}, groups=["session"]
    )
...
def test_a_distribution_kind_is_selectable_from_the_opening_state(series_table):
    ...
    so there is no sample — and the kind whose selection would supply one
    ...
    assert report["has_sample"] is False, "as the spec stands"
    ...
    assert Role.COLLAPSE.value in (kinds["violin"]["assignment"] or {})["roles"].values()


def test_a_kind_with_no_suggestion_is_still_judged_on_the_current_roles(series_table):
    """BAND needs a sample and does not change the shape, so it brings no
    ...
    assert "sample" in (kinds["band"]["reason"] or "")
