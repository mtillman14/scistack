...
| F38 | low | two owners | "which call sites form one pipeline step" (function + variable wiring) is decided twice: `scidb.inspect.graph.PipelineGraph` (CLI) and `graph_builder.group_call_sites_by_wiring` (GUI); each has its own grouping key | found fixing F19 |
