...
# ---------------------------------------------------------------------------
# Run markers — how a terminal-dispatched run reports that it started and how
# it ended. The format and the directory belong to ``scidb.run_markers``; the
# writer is ``scimatlab``'s ``+scidb/run_marker.m``. See
# ``.claude/plan-matlab-run-completion.md``.
# ---------------------------------------------------------------------------


def _run_marker_begin_lines(db_path: str, run_id: "str | None") -> list[str]:
    """Announce the run — emitted BEFORE the pyenv preamble.

    Before, not after, and that ordering is the whole point of this marker:
    the preamble rethrows, and a misconfigured ``pyenv`` is the commonest way
    these scripts die. A ``.started`` written after it could never
    distinguish "MATLAB never launched the script" from "the script died in
    setup" — and those have completely different fixes.
...
def _run_marker_finish_lines(
    db_path: str, run_id: "str | None", *, ok: bool, indent: str = ""
) -> list[str]:
    """Report the outcome. ``ok=False`` carries MATLAB's own error.

    The failure form reads ``scistack_err__``, so it is only valid inside the
    generated ``catch`` block — which is exactly where a failure is known
    precisely, identifier and message included. The success form goes after
    the ``try``/``catch`` ends, which the catch's ``rethrow`` makes
    ...
    dir_ = _escape_matlab_string(str(marker_dir(db_path)))
    rid = _escape_matlab_string(run_id)
    if ok:
        return [
            f"{indent}scidb.run_marker('finish', '{dir_}', '{rid}', true);",
            # Clearing runs the cleanup object, which finds the .done above
            # and does nothing — the point is to leave no junk in the user's
            # base workspace, since run() evaluates there.
            f"{indent}clear scistack_run_marker__;",
        ]
    return [
        f"{indent}scidb.run_marker('finish', '{dir_}', '{rid}', false, ...",
        f"{indent}    scistack_err__.identifier, scistack_err__.message);",
    ]
...
def generate_matlab_command(
    ...
    lines.append("")
    lines.extend(_run_marker_begin_lines(db_path, run_id))
    ...
    if not variants:
        ...
        lines.append("    end")
        # The first-run branch is a REAL run — it is what a never-yet-run
        # MATLAB node executes — so it reports like any other. Without this
        # it would write .started and never .done, and every first run would
        # resolve as "unknown".
        lines.extend(
            _run_marker_finish_lines(db_path, run_id, ok=False, indent="    ")
        )
        ...
        lines.extend(_run_marker_finish_lines(db_path, run_id, ok=True))
        return "\n".join(lines)
