...
- One owner: `scidb.parameter.declared_input_names(inputs, explicit)`
...
| F2 | high | coupling — **fixed 2026-09-23: seam module `scidb.external_loop` (public re-exports); bridge calls only it; AST guard test. With F7 and the rest: private cross-package imports 11 -> 0; tests unrun** | MATLAB for_each built from 7 private scidb.foreach steps | §1.4 |
...
| F4 | med | structure — **prepare split 2026-09-23 (.claude/plan-split-for-each-prepare.md): 146-line sequence of 8 named stages + `_RidIndex`; dry run now shares `_existing_combos` (its duplicate prefilter/exclusion copy deleted); size guard test; tests unrun. scifor `for_each` not yet touched** | `_for_each_prepare` 1,522 lines; scifor `for_each` 1,039 lines / 29 params | §1.3 |
...
| F7 | med | coupling — **fixed 2026-09-23: 8 sciduckdb converters made public (clean rename, no aliases); also `database.set_current_database`/`clear_current_database`, `pipeline.all_pipelines()`, scifor `merge_parts`, scistackplot `lazy_attr`** | scidb depends on 8 private sciduckdb converters | §1.4 |
...
| F10 | low | readability — **layout hop cut 2026-09-23: 7 db-less `layout.*` forwarders deleted; callers use `pipeline_store.*(db, …)` with the db they were handed (put_edge, _build_graph, ensure_node_identities and both MATLAB generators had been reading edges/pending values via `get_db()` beside a passed `db`); pending-constant handlers now take db; test_layout_service_db_routing.py; tests unrun. pipeline_store→intent_store hop KEPT: those are not pure — each call runs `_ensure_tables`, the only thing that creates `_intent`** | GUI forwarding chains (3 hops to intent_store) | §1.9 |
...
| F15 | med | perf — **measured 2026-09-24 (11.1 s = pipeline_variants 4.8 s + node state 6.1 s, of which expected_invocations 5.1 s, discovery_gate 1.0 s). Fixed: both N+1 loops read `invocation_edges_batch` / `invocation_outputs_batch` (one owner of edge-row interpretation); Plot Studio pickers call `get_pipeline(run_states=False)` and skip the state check; expected_invocations sub-phase timings added. RE-MEASURED 2026-09-24: canvas 11.1 s -> 2.5 s, picker 11.1 s -> 0.31 s (pipeline_variants 0.15 s, variant_configs 0.08 s). Then `realized_inputless_invocations` (0.85 s, 2 queries per invocation) -> one query. Left: discovery_gate ~0.9 s = one SMB walk per build (~340 stats); `state.DISCOVERY_CACHE_SECONDS` = 5 s memo is shorter than the gap between builds — raising it is a freshness trade-off for the user** | `get_pipeline` 10 s after a run (real data) | §3 |
| F16 | med | perf (N+1) — **fixed 2026-09-24 with F15: 4 queries per invocation -> 2 batched reads; same N+1 was in `function_variant_configs` (per call site)** | `provenance_query.pipeline_variants` runs ~4 queries per invocation — likely F15 | found fixing B1 |
...
| F19 | low | rival converter — **fixed 2026-09-23: scidb pure `aggregate_pipeline_variants` + GUI pure `aggregate_from_scidb`; test-only `aggregate_variants` deleted, tests compose the production pair; tests unrun** | `graph_builder.aggregate_variants` is test-only; production uses `api/pipeline.build_aggregate` | found fixing B1 |
| F20 | med | owner gap — **fixed 2026-09-24 (.claude/plan-f20-name-keyed-state.md): 4 readers compared declared Parameter names with argument keys (apply_pending_overrides, merge_pending_constants, resolve_target_call_id, and filter_hidden_constant_value_targets via binding-less history targets); one translation `edge_resolver.declared_by_argument`/`by_argument`; history targets now carry Parameter bindings from recorded names; all tests pass 2026-09-24** | pending constants were looked up by argument name in the pending-row synthesis (fixed with B1); audit other name-keyed GUI state for the same split | found fixing B1 |
...
| F30 | low | perf — **built 2026-09-24 (branch perf/fast-canonical-hash): measured 0.75 ms/record = 66% pandas column access + per-scalar json.dumps; fast column path (verified per signature) + same-bytes scalar dispatch, pinned to a frozen serializer copy; measured 0.746 -> 0.177 ms/record (4.2x) wide, 0.164 -> 0.074 tall** | `save_batch` per-row canonical hashing ~3 ms/record | §4.3 |
| F31 | low | observability — **instrumented 2026-09-24: `[timing] for_each_result_transfer` in +scidb/for_each.m (per output: shape, bytes, seconds, 3 slowest columns + to_python strategy); `to_python` returns per-column stats as an optional 2nd output; MATLAB tests added, unrun; awaiting a real MATLAB run** | MATLAB→Python result transfer unlogged (4.3 s for 450×59) | §4.3 |
...
| F35 | med | feature gap — **implemented 2026-09-23: hash_function.m returns the hashed .m text; for_each.m sends it; the bridge attaches it to the sentinel only if it re-hashes to the digest; the save stores it via function_sources_for; tests unrun** | MATLAB function source is never captured (`_function_source` rows exist only for Python): nothing sends the `.m` text. Needed for any code-version view of MATLAB functions (variant-selection). Bridge-side: send `source_text` with the digest, verify the digest matches before storing | found fixing F13 |
...
| F37 | high | wrong identity | scidb `get_aggregated_variants` keyed PathInputs by PARAMETER name: two functions with same-named PathInput params (`filePath`) but different templates were merged under the first template, so the second resolved to the first's declared PathInput (fixed 2026-09-23: keyed by spec with (call site, parameter) pairs; tests unrun) | found fixing F19 |
| F38 | low | two owners — **fixed 2026-09-24 (.claude/plan-f38-step-grouping.md, option A): runs record the DECLARED PathInput name on the PathInput edge (`declared_input_names` now owns Parameter + PathInput names); `database.call_site_wiring_ids` is the one step rule, CLI groups through it (outputs now split steps, as on the canvas); GUI prefers the recorded name over template matching; parity test; all tests pass 2026-09-24** | "which call sites form one pipeline step" (function + variable wiring) is decided twice: `scidb.inspect.graph.PipelineGraph` (CLI) and `graph_builder.group_call_sites_by_wiring` (GUI); each has its own grouping key | found fixing F19 |
| M1 | — | migrations removed | 2026-09-23 (user decision: beta, no databases to upgrade): scidb provenance ALTER backfills (5), GUI legacy `_pipeline_*` tables + `intent_store` one-time imports, JSON-layout→DuckDB migration, layout flat/scoped/placement migrations, pre-grouping position/edge adoption, `_pipeline_nodes.config` fallback, `_fold_indexed_params` legacy-history repair. Column meanings moved onto the CREATE TABLEs | review #10 |
